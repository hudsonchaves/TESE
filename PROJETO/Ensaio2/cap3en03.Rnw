...
{ P }_{ t }^{ * }=\frac { \eta  }{ \eta -1 } \frac { \sum _{ j=0 }^{ J-1 }{ { S }_{ j }{ E }_{ t }\left[ { Q }_{ t,t+j }{ Y }_{ t+j }{ P }_{ t+j }^{ \eta -1 }{ MC }_{ t+j } \right]  }  }{ \sum _{ j=0 }^{ J-1 }{ { S }_{ j }{ E }_{ t }\left[ { Q }_{ t,t+j }{ Y }_{ t+j }{ P }_{ t+j }^{ \eta -1 } \right]  }  } 
\end{equation}
...
{ P }_{ t }={ \left( \sum _{ j=0 }^{ J-1 }{ \theta (j){ P }_{ t-j }^{ *1-\eta  } }  \right)  }^{ \frac { 1 }{ 1-\eta  }  }
\end{equation}
...
{ \hat { p }  }_{ t }^{ * }={ E }_{ t }\left[ \sum _{ j=0 }^{ J-1 }{ \frac { { \beta  }^{ j }S(j) }{ \Omega  }  } \left( { \hat { mc }  }_{ t+j }+{ \hat { p }  }_{ t+j } \right)  \right] 
\end{equation}

\noindent onde $\Omega =\sum _{ j=0 }^{ J-1 }{ { \beta  }^{ j }S(j) }$ e ${ \hat { mc }  }_{ t }=\left( \delta +\phi  \right) { \hat { y }  }_{ t }-\left( 1+\phi  \right) { \hat { z }  }_{ t }$. De um modo semelhante, é possível derivar o log dos desvios do preço agregado através da loglinearização da equação~\ref{eq18en02}.

\begin{equation}\label{eq20en02}
{ \hat { p }  }_{ t }=\sum _{ k=0 }^{ J-1 }{ \theta (k){ \hat { p }  }_{ t-k }^{ * } } 
\end{equation}
...
{ \hat { \pi  }  }_{ t }=\sum _{ k=0 }^{ J-1 }{ \frac { \theta (k) }{ 1-\theta (0) } { E }_{ t-k }\left( \sum _{ j=0 }^{ J-1 }{ \frac { { \beta  }^{ j }S(j) }{ \Psi  } { \hat { mc }  }_{ t+j-k }+\sum _{ i=1 }^{ J-1 }{ \sum _{ i=1 }^{ J-1 }{ \frac { { \beta  }^{ j }S(j) }{ \Psi  }  }  } { \hat { \pi  }  }_{ t+i-k } }  \right) -\sum _{ k=2 }^{ J-1 }{ \Phi (k){ \hat { \pi  }  }_{ t-k+1 } }  } 
\end{equation}
...
{ I }_{ t }={ I }_{ t-1 }^{ { \rho  }_{ i } }{ \left[ { \left( \frac { { P }_{ t } }{ { P }_{ t-1 }\tilde { \pi  }  }  \right)  }^{ { \phi  }_{ \pi  } }{ \left( \frac { { Y }_{ t } }{ { Y }_{ t-1 } }  \right)  }^{ { \phi  }_{ y } } \right]  }^{ 1-{ \rho  }_{ i } }{ e }^{ { q }_{ t } }
\end{equation}
...
{ \hat { \pi  }  }_{ t }=\sum _{ k=0 }^{ J-1 }{ { W }_{ 1 }(k){ E }_{ t-k }\left( \sum _{ j=0 }^{ J-1 }{ { W }_{ 2 }(j){ \hat { mc }  }_{ t+j-k }+\sum _{ i=1 }^{ J-1 }{ { W }_{ 3 }(i){ \hat { \pi  }  }_{ t+i-k } }  }  \right) - } \sum _{ k=2 }^{ J-1 }{ { W }_{ 4 }(k){ \hat { \pi  }  }_{ t+1-k } } 
\end{equation}

\begin{equation}\label{eq24en02}
{ \hat { mc }  }_{ t }=\left( \delta +\phi  \right) { \hat { y }  }_{ t }-\left( 1+\phi  \right) {\hat{z}}_{t}
\end{equation}

\begin{equation}\label{eq25en02}
\delta { E }_{ t }\left[ { \hat { y }  }_{ t+1 } \right] =\delta { \hat { y }  }_{ t }+\left( { \hat { \iota  }  }_{ t }-{ E }_{ t }\left[ { \hat { \pi  }  }_{ t+1 } \right]  \right) +{ d }_{ t }
\end{equation}

\begin{equation}\label{eq26en02}
{ \hat { \iota  }  }_{ t }=\left( 1-{ \rho  }_{ i } \right) \left( { \phi  }_{ \pi  }{ \hat { \pi  }  }_{ t }+{ \phi  }_{ y }\left( { \hat { y }  }_{ t }-{ \hat { y }  }_{ t-1 } \right)  \right) +{ \rho  }_{ i }{ \hat { \iota  }  }_{ t-1 }+{ q }_{ t }
\end{equation}
...
{Y}_{0}\left(\mu \right){S}_{t}={Y}_{1}\left(\mu  \right){S}_{t-1}+{Y}_{\epsilon}\left(\mu\right){\epsilon_{t}} +{Y}_{\omega  }\left(\mu  \right) {\omega}_{t}
\end{equation}
...
{ S }_{ t }={ \Psi  }_{ 1 }\left( \mu  \right) { S }_{ t-1 }+{ \Psi  }_{ \epsilon  }\left( \mu  \right) { \epsilon  }_{ t }
\end{equation}

O segundo passo envolve escrever o modelo na forma de espaço de estados. Para tanto, acrescentamos à equação de solução ~\ref{eq31en02} uma equação de medida, que relationa as variáveis teóricas ao vetor de $Y_obs{t}$ observáveis. 

\begin{equation}\label{eq32en02}
{ Y\_ obs }_{ t }=A\left( \mu  \right) +B{ S }_{ t }+{ CV }_{ t }
\end{equation}

\noindent onde $A\left( \mu  \right)$ é um vetor de constantes capturando a média de $S_{t}$ e $V_{t}$ é um conjunto de choques às observáveis, incluindo medias de erro. 

Terceiro, quando assume-se que todos os choques na forma de espaço de estados são normalmente distribuídos, pode-se usar o filtro de Kalman (\citet{sargent1989two}) para avaliar a função de verossimilhança das observáveis ($\mu|Y\_ obs^{T}$). Em contraste aos métodos de máxima verossimilhança, a abordagem Bayesiana combina a função de verossimilhança com as densidades a priori $p\left( \mu  \right)$, que inclui todas as informações extra sobre os parâmetros de interesse. A densidade à posteriori $p\left( \mu |{ Y\_ obs }^{ T } \right)$ pode ser obtida pela aplicação do Teorema de Bayes:

\begin{equation}\label{eq33en02}
p\left( \mu |{ Y\_ obs }^{ T } \right) \left( \mu |{ Y\_ obs }^{ T } \right) p\left( \mu  \right) 
\end{equation}

No último passo, $\mu$ é estimado pela maximização da função de verossimilhança dados os dados ($(\mu |{ Y\_ obs }^{ T } \right)$) reponderada pela densidade à priori $p\left( \mu  \right)$, em que métodos de otimização numérica são usados para encontrar a posteriori para $\mu$ e a inversa da matriz Hessiana. Finalmente, a distribuição a posteriori é gerada por meio do uso do algorítmo Metropolis.
